...
<assert test="parent::tbx:termSec or parent::tbx:termNoteGrp/parent::tbx:termSec">Any termNote is only allowed at the termSec level.</assert>
    ...
    <pattern id="dialectEnforcement">
        ...
        <rule context="tbx:admin/@type">
            <assert test=".='customerSubset' or .='projectSubset' or .='source'">Permited type values are 'customerSubset', 'projectSubset', or 'source'.</assert>
        </rule>
        <rule context="tbx:descrip/@type">
            <assert test=".='subjectField' or .='context' or .='definition'">Permitted type values are 'subjectField', 'context', or 'definition'.</assert>
        </rule>
        <rule context="tbx:termNote/@type">
            <assert test=".='administrativeStatus' or .='partOfSpeech' or .='geographicalUsage' or .='grammaticalGender' or .='termLocation' or .='termType'">Permitted type values are 'administrativeStatus', 'partOfSpeech', 'grammaticalGender', 'termLocation', or 'termType'</assert>
        </rule>
        <rule context="tbx:transac/@type">
            <assert test=".='transactionType'">Permitted type value is 'transacationType'.</assert>
        </rule>
        <rule context="tbx:ref/@type">
            <assert test=".='crossReference'">Permitted type value is 'crossReference'.</assert>
        </rule>
        <rule context="tbx:xref/@type">
            <assert test=".='externalCrossReference' or .='xGraphic'">Permitted type value is 'xGraphic'.</assert>
        </rule>
    </pattern>
